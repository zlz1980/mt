...
/**
 * 主动回滚DB事物，要求流程前序使用了DB事务开启的原子交易
 * 将中间所有对数据库的操作进行DB事务回滚。
 *
 * @author JiangTaiSheng
 */

@Atom("base.TransactionRollback")
public class TransactionRollbackImpl implements AtomService {

    private static final Logger LOGGER = LoggerFactory.getLogger(TransactionRollbackImpl.class);

    private final DataSourceTransactionManager transactionManager;

    public TransactionRollbackImpl(DataSourceTransactionManager transactionManager) {
    ...
    public AtomResult doService(TranContext tranContext, ScopeValUnit scopeValUnit, FlowUnit flowUnit) {
        ...
        try {
            ...
            LOGGER.error(APP_ATOM_RUN_ERR_KEY + "回滚数据库事务异常,异常信息[{}]", e.getMessage());
            throw new FlowException(T_C0003095.getCode(), tranContext.getFeTranCode(), T_C0003095.getCodeMsg("回滚数据库事务异常,异常信息[{}]", e.getMessage()), e);
        } finally {
            tranScope.remove(TRANSACTION_STATUS);
        }
        LOGGER.info("数据库事务回滚成功!");
