 ...
 * 第一行:卡号  acctNo|
 * 第二行:卡序列号 cardSeqNum|
 * 第三行: 应用组成的arqc数据 icArqcData|
 * 第四行:应用交易记数器9F36   processGene|
 * 第五行:待验证的ARQC(应用密文9F26)  arqcData|
 * 第六行:授权响应码(2位，银联应答码,生成ARPC必送)  arc|
 * 第七行:分散秘钥索引9F10第3到4位作为“分散密钥索引” keyIndex|
 * 第八行:算法标识9F10第15到16位作为算法标识   algFlag|
 * 第九行:密文版本号  01 -接触式   17 -非接触式   cryptoVersion|
 *
 * @Author :
 * @create 2025/2/10 14:18
 */
@Atom("fbsp.ARPCGenerate")
public class ARPCGenerateImpl implements AtomService {

    public static final String PROCESS_GENE = "processGene";
    public static final String IC_ARQC_DATA = "icArqcData";
    public static final String CRYPTO_VERSION = "cryptoVersion";
    public static final String ARQC_DATA = "arqcData";
    public static final String ARC = "arc";
    public static final String KEY_INDEX = "keyIndex";
    public static final String ALG_FLAG = "algFlag";
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ARPCGenerateImpl.class);
    @Autowired
...
//    private ARQCARPCApi arqcarpcApi;
    ...
    public AtomResult doService(TranContext tranContext, ScopeValUnit scopeValUnit, FlowUnit flowUnit) {
        ...
        if (paramList.size() != 9) {
            LOGGER.error(APP_ATOM_RUN_ERR_KEY + " 增强处理参数个数不为9,请检查参数配置[{}]", paramList);
            throw new FlowException(T_C0003092.getCode(), T_C0003092.getCodeMsg("增强处理参数个数不为9,请检查参数配置[{}]", paramList));
        }
        //日志输出增强处理参数信息
       ...
       00代替。如果应用主账号和应用主账号序列号的长度不等于16个数字：如果长度小于16个数字，右对齐，前面补0；如果长度大于16个数字，取最右边16个数字。*//*
        ...
        //取得授权响应码
        String arc = (String) reqScopeValUnit.get(ARC);
        //分散密钥索引
        ...
        try {
            ...
            //生成GenerateARPC
            String arpc = esscApi.CebGenerateARPCUsingDerivedKey(keyName, keyVersion, panAndCardSeqNum, processGene, icArqcDataStr, arqcData, arc, 0);
            //把arc转换大写16进制
            String arcHex = Hex.encodeHexString(arc.getBytes(), false);
            LOGGER.info("ARPC生成成功!");
            MapUtils.setVal(scopeValUnit, "ARPC", arpc + arcHex);
        } catch (Exception e) {
            LOGGER.error(APP_ATOM_RUN_ERR_KEY + "生成ARPC异常,异常信息[{}]", e.getMessage(), e);
            throw new FlowException(T_C0003092.getCode(), tranContext.getFeTranCode(), T_C0003092.getCodeMsg("生成ARPC异常,异常信息[{}]", e.getMessage()), e);
        }
