...
@Atom("fbsp.EcsAmountConvert")
public class EcsAmountConvertImpl implements AtomService {

    private static final Logger LOGGER = LoggerFactory.getLogger(EcsAmountConvertImpl.class);
    ...
    public AtomResult doService(TranContext tranContext, ScopeValUnit scopeValUnit, FlowUnit flowUnit) {
        ...
        // 连通传统authTransMessageDto.transAt
        ...
        if (!transAt.matches("^[0-9]+$")) {
            ...
            throw new FlowException(T_C0003092.getCode(), tranContext.getFeTranCode(), T_C0003092.getCodeMsg("金额栏位[{}]对应的金额值[{}]格式错误,请检查参数", atomTranParams[0], transAt), null);
        }
        try {
            ...
            LOGGER.info("连通传统渠道金额转换完成,原金额[{}],借记卡核心金额[{}]:[{}],贷记卡核心金额[{}]:[{}]", transAt, yKey, yTransAt, cKey, cTransAt);
        } catch (Exception e) {
            LOGGER.error(APP_ATOM_RUN_ERR_KEY + "连通传统渠道金额转换异常,异常信息[{}]", e.getMessage(), e);
            throw new FlowException(T_X0005091.getCode(), tranContext.getFeTranCode(), T_X0005091.getCodeMsg("连通传统渠道金额转换异常,异常信息[{}]", e.getMessage()), e);
        }
